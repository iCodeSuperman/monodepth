...
class MonodepthDataloader(object):
    ...
    def __init__(self, data_path, filenames_file, params, dataset, mode):
        ...
        if mode == 'train':
            ...
            print("================================ replace new method ===========================")
            my_dataset = tf.data.Dataset.from_tensor_slices([left_image, right_image]).shuffle(
                buffer_size=min_after_dequeue).batch(params.batch_size)
            iterator = my_dataset.make_initializable_iterator()
            with tf.Session() as sess:
                print("=================== run iterator.initializer =================")
                sess.run(iterator.initializer)
                while True:
                    try:
                        ts = sess.run(iterator.get_next())
                        left_image_batch = ts.__getitem__(0)
                        right_image_batch = ts.__getitem__(1)
                    except tf.errors.OutOfRangeError:
                        break
            print("left_image_batch: ", left_image_batch)
            print("right_image_batch: ", right_image_batch)
